            ...
            <div className="pt-1">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                {mode !== 'reset' && <div className="relative">
                  <input name="password" type={showPassword ? 'text' : 'password'} minLength={mode === 'signup' ? 6 : 6} maxLength={mode === 'signup' ? 15 : undefined} value={formData.password} onChange={(event) => setFormData((current) => ({ ...current, password: event.target.value }))} required placeholder="Password" autoComplete={mode === 'signup' ? 'new-password' : 'current-password'} className="w-full rounded-lg border border-[#E5E4E0] px-3 py-2 pr-10 text-sm outline-none focus:border-[#9C6644]" />
                ...
                {mode === 'signup' && <p className="text-xs leading-5 text-[#4A5568]">Use 6–15 characters with uppercase and lowercase letters, a number, and a special character.</p>}
